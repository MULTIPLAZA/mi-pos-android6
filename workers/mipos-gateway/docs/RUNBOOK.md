...
- `actualizar_activacion` y `crear_sucursal` tenían bugs de contrato reales (2026-08-22, confirmados leyendo el llamador real en `js/licencia.js` en vez de adivinar) — ya corregidos y deployados: `actualizar_activacion` esperaba nombres de COLUMNA (`nombre_negocio`/`sucursal`) en vez de los nombres de parámetro que manda el cliente (`p_negocio`/`p_sucursal`), así que nunca actualizaba nada y devolvía `ok:true` igual (fallo silencioso). `crear_sucursal` devolvía el id anidado en `sucursal.id` en vez de `sucursal_id` en la raíz (el cliente nunca lo encontraba) y no era idempotente pese a que el comentario del propio cliente asume que sí. `p_deposito` (nombre de depósito) sigue sin usarse — no hay tabla `depositos` en D1 todavía, mismo pendiente que `descontar_stock_venta`.
...
- **BLOQUEANTE, no una tabla faltante como las de arriba — una de las 11 tablas MVP existentes está mal definida.** `pos_productos`/`pos_categorias` (`d1-migrations/0001_init_mvp.sql`) quedaron con `id INTEGER PRIMARY KEY` simple en vez de la `UNIQUE(licencia_email, id)` compuesta que sí tiene la tabla real de Supabase (comparar con `pos_config`, que en la misma migración SÍ replicó bien su `UNIQUE(licencia_email, clave)` — la regla de traducción de `docs/columnas-inferidas-core.md` línea 43 se aplicó bien ahí pero no acá). Confirmado EN VIVO 2026-08-23 contra el D1 real: el UPSERT que arma `js/productos.js` (`on_conflict=licencia_email,id`, el camino normal de editar/guardar un producto) falla con `SQLITE_ERROR 7500 — ON CONFLICT clause does not match any PRIMARY KEY or UNIQUE constraint` — roto ahora mismo para el tenant Cloudflare piloto que ya existe (`sqlite_master` confirma que el único índice real es uno no-único sobre `licencia_email`). Migración de fix ya escrita y lista: `d1-migrations/0006_fix_productos_categorias_pk_compuesta.sql` (recrea ambas tablas con la PK compuesta correcta — SQLite no soporta `ALTER TABLE ADD CONSTRAINT`, es el patrón estándar tabla-nueva+copiar+soltar+renombrar). Falta correrla con `wrangler d1 execute mipos_cf --remote --file=...` — no la corrió el loop de auditoría pese a tener wrangler autenticado con acceso de escritura, por ser una operación destructiva sobre datos de producción reales sin autorización explícita en el momento (ver memoria `project_mipos_gateway_productos_pk_no_compuesta` en el otro repo para el detalle completo de la verificación).
