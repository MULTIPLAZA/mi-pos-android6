...
- `descontar_stock_venta` (en `src/rpc.js`) sigue sin implementar — tira 501 a propósito. La tabla `pos_productos` sí existe en D1 (0001_init_mvp.sql) pero falta confirmar el nombre exacto de la columna de stock y la semántica exacta contra `stockDescontarVenta` (cliente) antes de dar esto por terminado (descuento de stock es un dato financiero, no vale la pena adivinar).
