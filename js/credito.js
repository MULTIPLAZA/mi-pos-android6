...
function cliSaldo(clienteId) {
  ...
  for (var i = 0; i < fiados.length; i++) {
    if (fiados[i].clienteId === clienteId) totalFiado += (fiados[i].total || 0);
  }
  var cobros = cobrosCargar();
  var totalCobrado = 0;
  for (var j = 0; j < cobros.length; j++) {
    if (cobros[j].clienteId === clienteId) totalCobrado += (cobros[j].monto || 0);
  }
  return Math.max(0, totalFiado - totalCobrado);
}
...
function fiadoCobrar(clienteId, montoGs, metodoPago) {
  ...
  return montoGs; // retornar siempre el monto solicitado — pagos parciales son cobros válidos
}
...
function renderCreditoScreen() {
  ...
  html += '<div><div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:.8px;font-weight:700;margin-bottom:3px;">Total pendiente</div><div style="font-size:26px;font-weight:900;color:#e53935;letter-spacing:-1px;">'+numGs(totalPend)+'</div></div>';
  ...
  for (var ii = 0; ii < conSaldo.length; ii++) {
    ...
    if (c.limiteGs > 0) {
      ...
      html += '<div style="font-size:10px;color:'+(pct>=90?'#e53935':pct>=70?'#ff9800':'var(--muted)')+';">Límite '+numGs(c.limiteGs)+'</div>';
    }
    html += '</div></div>';
  }
...
function abrirCobrarFiado(clienteId) {
  ...
  document.getElementById('cobrarFiadoSaldo').textContent  = numGs(_cobFiadoSaldo);
  document.getElementById('cobrarFiadoMonto').value        = '';
  document.getElementById('cobrarFiadoBtnTodo').textContent = 'PAGAR TODO — '+numGs(_cobFiadoSaldo);
...
function guardarEditCli() {
  ...
  if (_editCliId) {
    ...
    var c = cliNuevo(nombre, lim);
    if(typeof toast==='function') toast('Cliente creado');
    cerrarEditCli();
    if (_nuevoClienteDesdeCobroCb) {
      // Vino desde "Nuevo" en el picker de cobro — seleccionar y abrir picker
      _nuevoClienteDesdeCobroCb = false;
      abrirClientePicker();
...
function seleccionarClienteCredito(clienteId) {
  ...
  if (c.limiteGs > 0 && afterSale > c.limiteGs) {
    if (!confirm(c.nombre+': el saldo llegaría a '+numGs(afterSale)+', superando el límite de '+numGs(c.limiteGs)+'.\n¿Continuar de todas formas?')) return;
  }
  creditoClienteSel = c;
...
function nuevoClienteRapido() {
  // Guardar callback para seleccionar el cliente después de guardarlo
  _nuevoClienteDesdeCobroCb = true;
  cerrarClientePicker();
  abrirNuevoCliente();
}
var _nuevoClienteDesdeCobroCb = false;
...
function updCreditoSecUI() {
  ...
  if (creditoClienteSel) {
    ...
    if (saldoEl) {
      ...
      saldoEl.textContent = s > 0 ? 'Saldo actual: '+numGs(s) : 'Sin deuda previa ✔';
