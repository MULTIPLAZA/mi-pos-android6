// ════════════════════════════════════════════════════════════
// NODO POS — lib compat (sin ES modules)
// Reemplaza js/lib/index.mjs para Chrome < 61 (Android 6).
// Expone los mismos globals que el módulo ESM original.
// ════════════════════════════════════════════════════════════
...
// ── gs — formato guaraníes ────────────────────────────────────
window.gs = function(n) {
  return '₲' + Math.round(n || 0).toLocaleString('es-PY');
};

// ── NodoIco — iconos SVG inline ──────────────────────────────
var __NODO_ICO_PATHS = {
...
window.__NODO_ICO_PATHS = __NODO_ICO_PATHS;

window.NodoIco = function(name, size, color) {
  var p = __NODO_ICO_PATHS[name];
  if (!p) return '';
  var s   = size  || 14;
...
try { window.dispatchEvent(new CustomEvent('nodo-lib-ready')); } catch(e) {}
